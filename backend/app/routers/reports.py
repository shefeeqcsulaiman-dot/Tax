...
from app.models import Account, AppDataRecord, AuditLog, Document, Employee, Invoice, Job, JournalEntry, JournalLine, PayrollRun, SourceTransaction, StockProductMapping, TaxCode, TaxLine, User, Warehouse
...
def dashboard(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> dict[str, Any]:
    ...
    invoice_count = count(db, Invoice, company_id)
...
def top_customers(db: Session, company_id: str) -> list[dict[str, str]]:
    ...
    return [{"name": name, "total": amount(money(total))} for name, total in rows]


def invoice_status(db: Session, company_id: str) -> dict[str, dict[str, str | int]]:
    total_count = int(db.query(func.count(Invoice.id)).filter(Invoice.company_id == company_id).scalar() or 0)
